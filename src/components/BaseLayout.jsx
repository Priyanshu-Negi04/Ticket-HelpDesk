...
export default function BaseLayout({ children }) {
  ...
  const sidebarLinks = {
    user: [
      { to: "/user/dashboard", label: "Dashboard", icon: "fas fa-th-large" },
      { to: "/user/new-ticket", label: "New Ticket", icon: "fas fa-ticket-alt" },
      { to: "/user/my-ticket", label: "My Ticket", icon: "fas fa-inbox" },
    ],
    operation: [
      { to: "/operation/dashboard", label: "Dashboard", icon: "fas fa-th-large" },
      { to: "/operation/ticket-approval", label: "Ticket Approval", icon: "fas fa-check-circle" },
      { to: "/operation/my-ticket", label: "My Ticket", icon: "fas fa-inbox" },
      { to: "/operation/performance", label: "Performance", icon: "fas fa-chart-line" },
    ],
    support: [
      { to: "/support/dashboard", label: "Dashboard", icon: "fas fa-th-large" },
      { to: "/support/my-ticket", label: "My Ticket", icon: "fas fa-inbox" },
      { to: "/support/performance", label: "Performance", icon: "fas fa-chart-line" },
    ],
    admin: [
      { to: "/admin/dashboard", label: "Dashboard", icon: "fas fa-th-large" },
      { to: "/admin/database", label: "Database", icon: "fas fa-database" },
      { to: "/admin/settings", label: "Settings", icon: "fas fa-cogs" },
      { to: "/admin/user-logs", label: "User Log History", icon: "fas fa-history" },
    ],
  };
  ...
  return (
    <div className="h-screen flex flex-col font-serif bg-gray-50">
      {/* Header */}
      <header className="flex items-center justify-between bg-[#5DE0C9] px-4 py-3 border-b border-[#2B9AE8] w-full h-15">
        <h1 className="italic font-bold text-white text-2xl select-none">Helpdesk</h1>
        <nav className="flex items-center space-x-4 text-black text-sm font-semibold select-none" style={{zoom : 1.15}} >
      ...
      <div className="flex flex-1 w-full overflow-hidden">
        ...
        <aside className="bg-gray-200 w-64 flex flex-col space-y-3 py-4 px-3 select-none border-r border-gray-300">
          <ul>
            {links.map(({ to, label, icon }) => (
              <li key={to} className="sidebar-item" style={{zoom : 1.15}}>
                <NavLink
                  to={to}
                  className={({ isActive }) =>
        ...
        <div className="w-full flex flex-col flex-1">
          {/* Content */}
          <main className="p-4 bg-white border-b border-[#2B9AE8] flex-1 overflow-auto">
            {children}
          </main>
          {/* Footer */}
